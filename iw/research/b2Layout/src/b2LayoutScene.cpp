...
{
	mStandard = ci::vec2(40, 50);
	mFeatured = ci::vec2(45, 60);
	mHighlight = ci::vec2(55, 75);

	/*mStandard = ci::vec2(15, 20);
	mFeatured = ci::vec2(17, 25);
	mHighlight = ci::vec2(27, 30);*/
	...
	mParams->addParam("center - x", &mCenter.x);
	mParams->addParam("center - y", &mCenter.y);
	mParams->addParam("bounds - x1", &mBounds.x1);
	mParams->addParam("bounds - y1", &mBounds.y1);
	mParams->addParam("bounds - x2", &mBounds.x2);
	mParams->addParam("bounds - y2", &mBounds.y2);

	mBounds = Rectf( vec2( getWindowSize() )*.03f, vec2(getWindowSize())*.97f );
...
void b2LayoutScene::populateInitialData()
{
	...
	{
		auto label = Rectf(vec2(0), vec2(randFloat(100, 150), randFloat(30,50)));
