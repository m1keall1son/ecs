...
Layout::Subcategory b2Layout::generate(const std::vector< std::pair< ci::Rectf/*label size*/, std::vector<ci::Rectf/*product size*/>>>& sub_category_data)
{
	...
	for (auto & cloud : sub_category_data) {
		...
		auto cloud_body = createCloud(num, new_bounds);
	...
	for (auto & cloud : mClouds) {
		...
		resetCloudBounds(cloud, in_order_size, true);
		label++;
	}
...
bool b2Layout::runUntilSleeping()
{
	int iterations = 0;
...
void b2Layout::resetProductRect(int cloud, b2Body * product, int priority, bool circle)
{
...
b2Body* b2Layout::createCloud(int index, const ci::Rectf & cloud_bounds)
{
	b2BodyDef bodyDef;
	bodyDef.type = b2_dynamicBody;
	bodyDef.linearDamping = 2.;
	...
	auto width = 2;
	auto height = mNumClouds / 2;

	x = (index % width) - width/2;
	y = floor(index / height) - height/2;

	auto step = vec2( (mBounds.getWidth()*.5f)/(float)width, (mBounds.getHeight()*.5f) / (float)height );
